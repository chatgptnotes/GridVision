...
interface EarthFaultRelaySymbolProps {
  width?: number;
  height?: number;
...
export default function EarthFaultRelaySymbol({ width = 50, height = 60, state = 'NORMAL', color, onClick, label }: EarthFaultRelaySymbolProps) {
  ...
  return (
    <svg width={width} height={height} viewBox="0 0 50 60" onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }} xmlns="http://www.w3.org/2000/svg">
      ...
      {/* ANSI number */}
      <text x={25} y={26} textAnchor="middle" dominantBaseline="central" fill={stateColor} fontSize={11} fontWeight="bold" fontFamily="Arial, sans-serif">51N</text>

      {/* Function indicator below number */}
      <text x={25} y={36} textAnchor="middle" fill={stateColor} fontSize={6} fontFamily="Arial, sans-serif">E/F</text>
      ...
      {state === 'OPERATED' && (
        <>
          <style>{`@keyframes earth-fault-relay-pulse { 0%,100%{stroke-opacity:1}50%{stroke-opacity:0.3} } .earth-fault-relay-operated{animation:earth-fault-relay-pulse 0.8s ease-in-out infinite}`}</style>
          <circle className="earth-fault-relay-operated" cx={25} cy={28} r={19} fill="none" stroke={stateColor} strokeWidth={2} strokeDasharray="3 2" />
        </>
      )}
