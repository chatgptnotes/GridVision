...
interface UndervoltageRelaySymbolProps {
  width?: number;
  height?: number;
...
export default function UndervoltageRelaySymbol({ width = 50, height = 60, state = 'NORMAL', color, onClick, label }: UndervoltageRelaySymbolProps) {
  ...
  return (
    <svg width={width} height={height} viewBox="0 0 50 60" onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }} xmlns="http://www.w3.org/2000/svg">
      ...
      {/* ANSI number */}
      <text x={25} y={26} textAnchor="middle" dominantBaseline="central" fill={stateColor} fontSize={12} fontWeight="bold" fontFamily="Arial, sans-serif">27</text>

      {/* Function indicator below number */}
      <text x={25} y={36} textAnchor="middle" fill={stateColor} fontSize={6} fontFamily="Arial, sans-serif">U/V</text>
      ...
      {state === 'OPERATED' && (
        <>
          <style>{`@keyframes undervoltage-relay-pulse { 0%,100%{stroke-opacity:1}50%{stroke-opacity:0.3} } .undervoltage-relay-operated{animation:undervoltage-relay-pulse 0.8s ease-in-out infinite}`}</style>
          <circle className="undervoltage-relay-operated" cx={25} cy={28} r={19} fill="none" stroke={stateColor} strokeWidth={2} strokeDasharray="3 2" />
        </>
      )}
